...
int main()
	...
	try
		...
		for (size_t i = 0; i < 10; i++)
			...
			scene.ecs_registry.emplace<iige::ecs::components::collider>(entity, utmg::circle{{0, 0}, 10});
			scene.ecs_registry.emplace<iige::ecs::components::has_collision<0>>(entity);

			}/**/

		scene.ecs_registry.emplace<iige::ecs::components::collides_with<0>>(first);


		scene.update();

		engine::Loop loop{engine::Loop::create<1>(scene, window, steps_per_second)};
		loop.run();
		}
